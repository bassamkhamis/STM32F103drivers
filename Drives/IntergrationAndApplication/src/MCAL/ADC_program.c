...
static volatile void (*ADC_u8CallBackPtr)(void) = NULL;
*/
...
void ADC_VoidInit(void)
{
	...
	MSTK_Delay_ms(1); //  1m sec delay

	//Turn ADC on for the second Time
	SET_BIT(ADC1 -> CR2,0); // set ADON bit
	SET_BIT(ADC1 -> CR2,1);
	MSTK_Delay_ms(1); // 1 ms delay

	// Set calibration Bit
	SET_BIT(ADC1 -> CR2,2);
	MSTK_Delay_ms(2); //2 ms delay


	/*//Enable End of conversion Interrupt
	SET_BIT(ADC1 -> CR1,5); //Enable End of conversion Interrupt
	//Enable Interrupt in the NVIC
	//NVIC_EnableIRQ(18);
	MNVIC_voidSetPendingFlag(18);*/
}
